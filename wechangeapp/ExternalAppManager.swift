...
struct ExternalAppManager {
    ...
    static func startExternalAppDialog(appInfo: ExternalAppInformation, fromViewController vc: UIViewController) {
        if let appLaunchURL = URL(string: appInfo.appURL) {
            ...
            if UIApplication.shared.canOpenURL(appLaunchURL as URL){
                externalAppIsInstalledDialog(appInfo: appInfo, presentingViewController: vc)
            } else {
                if Config.DEBUG == true { print("No App found for this URL: \(appLaunchURL)") }
                externalAppIsNotInstalledDialog(appInfo: appInfo, presentingViewController: vc)
            }
        }
    }
    ...
    private static func externalAppIsNotInstalledDialog(appInfo: ExternalAppInformation, presentingViewController vc: UIViewController){
